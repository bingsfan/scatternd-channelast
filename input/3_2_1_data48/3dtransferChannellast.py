...
def convert_to_channel_last(data, c, h, w,align_to=16):
    """
    将 CHW 格式的数据转换为 HWC 格式，并按 16 通道对齐存储。
    ...
    # 按 16 通道对齐存储
    num_groups = (c + padding_channels) // align_to  # 计算总组数
...
def main(data_path, c, h, w, output_path,align_to):
    """
    主函数：读取数据并处理。
    ...
    # 转换数据并按 16 通道对齐存储
    group_files = convert_to_channel_last(data, c, h, w,align_to)
...
if __name__ == "__main__":
    # 示例参数
    data_path = 'myoutput-origin.txt'  # 输入数据路径
    c = 48  # 通道数
    h = 4  # 高度
    w = 4  # 宽度
    align_to=16
    output_path = "myoutput-channellast16.txt"  # 最终合并文件的路径

    # 调用主函数
    main(data_path, c, h, w, output_path,align_to)